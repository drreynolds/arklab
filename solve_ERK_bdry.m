function [tvals,Y,nsteps,afails,ierr] = solve_ERK_bdry(fcn,StabFn,bdry,tvals,Y0,B,rtol,atol,hmin,hmax)
% usage: [tvals,Y,nsteps,afails,ierr] = solve_ERK_bdry(fcn,StabFn,bdry,tvals,Y0,B,rtol,atol,hmin,hmax)
%
...
for tstep = 2:length(tvals)
   ...
   while ((t-tvals(tstep))*h < 0)
      ...
      for stage=1:s
         ...
         % Compute new stage, enforce boundary conditions, and store appropriately
         Fdata.stage = stage;
         Znew = Calc(storage, Fdata);
         Znew = bdry(t+h*c(stage),Znew);
